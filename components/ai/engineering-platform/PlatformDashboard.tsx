    ...
    <div className="relative w-full bg-gray-900/70 rounded-xl overflow-hidden border border-gray-800 shadow-xl">
      ...
      <div className="bg-gray-800/90 p-3 flex items-center justify-between">
        ...
        <div className="flex space-x-3">
          <button 
            className={`px-3 py-1 rounded-md text-xs ${activeSection === "models" ? "bg-white/40 text-white/70" : "bg-gray-800 text-neutral-400"}`} 
            onClick={() => setActiveSection("models")}
          >
            AI Models
          </button>
          <button 
            className={`px-3 py-1 rounded-md text-xs ${activeSection === "observability" ? "bg-white/40 text-white/70" : "bg-gray-800 text-neutral-400"}`} 
            onClick={() => setActiveSection("observability")}
          >
            Observability
          </button>
          <button 
            className={`px-3 py-1 rounded-md text-xs ${activeSection === "analytics" ? "bg-white/40 text-white/70" : "bg-gray-800 text-neutral-400"}`} 
