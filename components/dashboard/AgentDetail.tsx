    ...
    <Dialog open={!!agent} onOpenChange={() => onClose()}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex border-b border-gray-800 mt-2">
          <button 
            className={`px-4 py-2 text-sm font-medium ${activeTab === 'overview' ? 'text-[var(--white)] border-b-2 border-[#ffffff]' : 'text-neutral-400 hover:text-[var(--white)]'}`}
            onClick={() => setActiveTab('overview')}
          >
            Overview
          </button>
          <button 
            className={`px-4 py-2 text-sm font-medium ${activeTab === 'rag' ? 'text-[var(--white)] border-b-2 border-[#ffffff]' : 'text-neutral-400 hover:text-[var(--white)]'}`}
            onClick={() => setActiveTab('rag')}
          >
            RAG Sources
          </button>
          <button 
            className={`px-4 py-2 text-sm font-medium ${activeTab === 'settings' ? 'text-[var(--white)] border-b-2 border-[#ffffff]' : 'text-neutral-400 hover:text-[var(--white)]'}`}
              ...
              <div className="col-span-1 border-r border-gray-800 pr-4">
                ...
                <div className="space-y-1">
                  {editedAgent.ragSources?.map(source => (
                    ...
                    >
                      <div className="flex items-center">
                        {source.type === 'database' && <Database className="h-4 w-4 text-[#ffffff] mr-2" />}
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        {(['database', 'vector', 'file', 'api'] as const).map(type => (
                          ...
                          >
                            {type === 'database' && <Database className="h-5 w-5 text-[#ffffff] mb-1" />}
