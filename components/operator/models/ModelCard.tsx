  ...
  return (
    <motion.div
      ...
      className={`
        ...
        ${model.recommended ? 'border-[#ffffff]/50' : 'border-gray-800'}
      `}
    >
      {model.recommended && (
        <span className="absolute top-0 right-6 -translate-y-1/2 px-3 py-1 bg-[#ffffff] text-[var(--white)] text-xs font-semibold rounded-full">
          Recommended
        </span>
      )}
      ...
      <ul className="space-y-2 mb-6">
        {model.features.map((feature, i) => (
          <li key={i} className="flex items-start">
            <Check className="h-5 w-5 text-[#ffffff] mr-2 flex-shrink-0 mt-0.5" />
