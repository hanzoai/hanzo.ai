...
const IAM_BASE_URL = process.env.NEXT_PUBLIC_HANZO_IAM_URL || 'https://id.hanzo.ai';
const CLIENT_ID = process.env.NEXT_PUBLIC_HANZO_CLIENT_ID || '';
const REDIRECT_URI = process.env.NEXT_PUBLIC_HANZO_REDIRECT_URI || `${(typeof window !== 'undefined' ? window.location.origin : '')}/auth/callback`;
...
export function getStoredAuthState(): AuthState {
  if (typeof window === 'undefined') {
    return { user: null, accessToken: null, refreshToken: null, idToken: null, isAuthenticated: false, isLoading: false, error: null };
  }
  try {
