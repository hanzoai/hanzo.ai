      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
          ...
          <div className="relative h-32 mt-2">
            <div className="absolute inset-0">
              <div className="h-full w-full flex">
                <motion.div 
                  className="h-full bg-[#fd4444]/70 rounded-l"
                  initial={{ width: 0 }}
                  animate={{ width: "42%" }}
                  transition={{ duration: 0.8, delay: 0.1 }}
                />
                <motion.div 
                  className="h-full bg-[#ff6b6b]/70"
                  initial={{ width: 0 }}
                  animate={{ width: "28%" }}
        ...
        <div className="bg-gray-800/40 border border-gray-700/40 rounded-lg p-3">
          ...
          <div className="h-32 relative">
            <div className="absolute bottom-0 w-full flex items-end space-x-1">
              {[35, 42, 38, 52, 45, 55, 47, 62, 48, 60, 53, 58].map((value, index) => (
                <motion.div
                  key={index}
                  className="bg-gradient-to-t from-[#fd4444]/90 to-[#ff6b6b]/50 rounded-t w-full"
                  style={{ height: `${value}%` }}
                  initial={{ height: 0 }}
          ...
          <div className="mt-1 text-xs text-neutral-300">
            $1,245 spent this month - <span className="text-[#fd4444]">14% under budget</span>
          </div>
        </div>
      </div>
